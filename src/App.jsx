...
export default function App() {
  ...
  function handleClick(event) {
    const clickedEmoji = event.target.textContent //butonun icerigini alalim

    // likedEmojis state'ini güncelleyelim
    setLikedEmojis((prevLikedEmojis) => [...prevLikedEmojis, clickedEmoji])

    // passedEmojis state'ini güncelleyelim
    const remainingEmojis = currentEmojis.filter(
      (emoji) => emoji !== clickedEmoji
    )
    ...
    // Yeni bir rastgele emoji listesi oluşturup, currentEmojis state'ini güncelleyelim
    setCurrentEmojis(getRandomEmojis())

    // LikedEmojis dizisinin uzunluğunu kontrol et ve gerekirse sonuçları göstermek için getResults fonksiyonunu çağıralim
    if (likedEmojis.length === 9) {
      getResults()
    }
  }
